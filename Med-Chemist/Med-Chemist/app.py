...
def signup():
    ...
    #Simple validation
    if not (username and password and cpassword and email and location and store):
    ...
    user_data = {'username': username, 'email':email , 'password': password, 'location': location , 'store':store}
...
def orders():
    ...
    if 'user_id' in session:
        ...
        return render_template('prescription.html', photos_data=photos_data,grouped_data=grouped_data,owl_carousel_css_link=owl_carousel_css_link,font_awesome_css_link=font_awesome_css_link,google_fonts_css_link=google_fonts_css_link)
    else:
...
def order_update():
    ...
    if 'user_id' in session:
        data = list(order_collection.find())  # Convert cursor object to list
        grouped_data = {}
        grand_totals = {}
        for item in data:
            if 'token_number' in item:
...
def news():
    ...
    newsletter_info.insert_one(data_to_insert)
